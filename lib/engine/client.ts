...
// instead of paying the latency of two wasted attempts before reaching the
// real answer. Does NOT catch the
...
export function anthropicClient(): Anthropic {
  ...
  // Bound each attempt at 60s (vs the SDK's 600s default). SDK retries are off
  // because Alpha's explicit, counted fallback/retry layers own that decision.
  // One logical paid-call reservation must never hide a second provider call.
  // A slow or overloaded API becomes a fast per-call failure the select-sections
  ...
  _client = new Anthropic({ apiKey, timeout: 60_000, maxRetries: 0 });
  return _client;
}
