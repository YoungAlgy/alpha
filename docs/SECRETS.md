...
| Ops alert webhook URL (Alpha-only Discord channel) | `ALPHA_OPS_ALERT_WEBHOOK_URL` | `ALPHA_OPS_ALERT_WEBHOOK_URL` | `SEND_ALPHA_OPS_ALERT_WEBHOOK_URL` (daily send, reconcile, and the watchdog's own alerts) | Discord is the primary ops channel. If unset, every alert falls back to email, signup pings (`sendOpsWebhookAlert`) go nowhere, and the watchdog fails its secrets check daily. The older `OPS_ALERT_WEBHOOK_URL` / `SEND_OPS_ALERT_WEBHOOK_URL` names are no longer read. |
