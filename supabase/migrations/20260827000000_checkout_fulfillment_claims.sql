...
create table public.checkout_creation_reviews (
  ...
  reason              text not null default 'replay_window_missed'
    check (reason = 'replay_window_missed'),
...
create table public.account_deletion_sagas (
  ...
  suppression_cleared_at    timestamptz,
...
begin
  update public.account_deletion_sagas
   ...
   where user_id = p_user_id
     ...
     and suppression_cleared_at is not null;
  return found;
end;
$$;
...
create or replace function public.mark_account_deletion_suppression_cleared(p_user_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  update public.account_deletion_sagas
     set suppression_cleared_at = coalesce(suppression_cleared_at, now()),
         updated_at = now()
   where user_id = p_user_id
     and state = 'billing_clean';
  return found;
...
revoke all on function public.mark_account_deletion_support_deleted(uuid)
  from public, anon, authenticated;
revoke all on function public.mark_account_deletion_suppression_cleared(uuid)
  from public, anon, authenticated;
...
grant execute on function public.mark_account_deletion_support_deleted(uuid)
  to service_role;
grant execute on function public.mark_account_deletion_suppression_cleared(uuid)
  to service_role;
