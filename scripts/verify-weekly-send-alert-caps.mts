...
console.log("(7) the stuck-claim RECLAIM block reports counts without subscriber identifiers");
{
  ...
  check("(7b) the reclaim log reports only the count", /RECLAIMED \$\{reclaimed!\.length\} stuck claim\(s\)/.test(src) && !src.includes("${idsLine}"));
  check("(7c) the reclaim alert points to protected records", src.includes("Use the protected issue records for exact subscribers."));
  check("(7d) no raw reclaimed user-id list is built", !src.includes("const idsLine") && !src.includes("user_id(s):"));
}
