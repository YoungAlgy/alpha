...
export function psqlEnvironment(connection, ambient = process.env) {
  const environment = {};
  ...
  for (const [key, value] of Object.entries(ambient || {})) {
    ...
    if (value !== undefined && systemKeys.has(normalizedKey)) {
  ...
  // Construct a small child environment instead of inheriting libpq, Node,
  // credential-file, or TLS settings. The SQL transaction independently
  // checks the server-side address before taking its first lock.
  return environment;
}
