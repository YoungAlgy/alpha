...
interface DigestProps {
  ...
  // (14:17 UTC) and their local calendar has already crossed into the next
  ...
  // against, not the real 14:17Z send time) -- corrected here to UTC+10 and
  // above, now that formatDateline anchors to the real send time.
...
// the REAL send time (lib/cadence.ts's SEND_HOUR_UTC and SEND_MINUTE_UTC, 14:17Z) -- this used
...
// used the real send-time anchor. Only the FORMAT step's timezone varies by
...
function formatDateline(weekOf: string, localTimezone: boolean): string {
  ...
  return new Date(`${weekOf}T${String(SEND_HOUR_UTC).padStart(2, "0")}:${String(SEND_MINUTE_UTC).padStart(2, "0")}:00Z`).toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
